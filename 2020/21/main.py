...
TEST_DISH2 = Dish(ingredients={"trh", "fvjkl", "sbzzf", "mxmxvkd"}, allergens={"dairy"})

assrt(TEST_DISH1, parse_line, TEST_STRING1)
assrt(TEST_DISH2, parse_line, TEST_STRING2)


def reconcile_dishes(dishes):
    ...
    while have_updated:
        ...
        for allergen, dishes in allergen_to_dish.items():
            if allergen in known_allergens:
                continue
            possible_ingredients = deepcopy(all_ingredients) - set(
                ingredients_with_known_allergens.keys()
            )
            for dish in dishes:
...
def produce_canonical_allegen_list(ingredients_with_known_allergens):
    return ",".join(
        ...
        for ingredient, _ in sorted(
            ingredients_with_known_allergens.items(), key=lambda x: x[1]
        )
    )
...
TEST_ALLERGENS = reconcile_dishes(TEST_DISHES)
...
def main():
    dishes = read_input()
    known_allergens = reconcile_dishes(dishes)
