...
class Op(NamedTuple):
    ...
    def run(self, memory, pc, inputs, outputs) -> int:
...
class Runner:
    def __init__(self, memory, inputs):
        ...
        self.inputs = deque()
        for i in inputs:
            self.inputs.appendleft(i)
        self.outputs = deque()

    def run(self):
        while self.pc != -1:
            op = read_op(self.memory[self.pc])
            self.pc = op.run(self.memory, self.pc, self.inputs, self.outputs)
        return self.memory, list(self.outputs)

def eval_program2(memory, inputs):
    runner = Runner(memory, inputs)
    return runner.run()
...
def find_highest_signal(memory):
    max_signal = 0
    for settings in permutations(range(5)):
        prev_output = 0
        for setting in settings:
            _, [prev_output] = eval_program2(memory, [setting, prev_output])
        max_signal = max(max_signal, prev_output)
    return max_signal

def find_highest_signal_with_loop(memory):
    max_signal = 0
...
def main():
    with open('in.txt') as infile:
        ...
        print(find_highest_signal(state))
        print(find_highest_signal_with_loop(state))
